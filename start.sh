...
echo $amiId
aws ec2 run-instances --image-id $amiId  --count 1 --instance-type t2.micro --key-name $keyPairName --security-group-ids $groupId --subnet-id $subnetId --tag-specifications "ResourceType=instance,Tags=[{Key=Name,Value=$1}]" --user-data file://$2
...
ssh -o StrictHostKeyChecking=no  -i ./$keyPairName.pem admin@$ipAddr
